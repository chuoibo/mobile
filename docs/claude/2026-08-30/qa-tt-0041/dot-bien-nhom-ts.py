...
def chay():
    p = subprocess.run(["bash", "-lc", CMD], cwd=TREE, capture_output=True, text=True, timeout=1800)
...
def main():
    ...
    for mid, need, path, old, new, why in MUTANTS:
        goc = path.read_text()
        if goc.count(old) != 1:
            bang.append((mid, need, "NEO-HONG", f"neo xuat hien {goc.count(old)} lan", why))
        ...
        print(f"{mid}: can {need}, do duoc {thuc} -> {dat}  ({f} ca do)  {ten[:70]}", flush=True)
    ...
    if not (co_xanh and co_do):
        print("BANG KHONG DUNG DUOC: khong co ca hai gia tri XANH va DO, "
