...
def layer_a() -> list[tuple[str, bool, str]]:
    ...
    for label, path, old, new, expect_red, why in LAYER_A:
        ...
        n = text.count(old)
        if n != 1:
            rows.append((label, False, f"neo xuất hiện {n} lần, cần đúng 1 — BỎ, không đo được"))
            continue
        backup = text
        try:
...
def layer_b() -> list[tuple[str, bool, str]]:
    ...
    def check(label, expect_red, why, mutate):
        ...
        red = rc != 0
        ok = red == expect_red
        rows.append((label, ok, f"{'ĐỎ' if red else 'XANH'} (cần {'ĐỎ' if expect_red else 'XANH'}) — {why}"))
    ...
    check("B1 chưa ai đi bộ bao giờ", True, "không có phán quyết KHÔNG phải là đạt",
          lambda: vp.unlink() if vp.exists() else None)
    check("B2 lượt gần nhất ĐỨT", True, "rc khác 0",
          lambda: write(rc=1, buoc_hong="QUET BILL: anh -> mon"))
    check("B3 phán quyết quá cũ", True, "quá ngưỡng 24 giờ",
          lambda: write(rc=0, ts=time.time() - 40 * 3600))
    check("B4 phán quyết về máy KHÁC", True, "khớp máy khác không nói gì về máy này",
          lambda: write(rc=0, ts=time.time(), url="http://127.0.0.1:9999"))
    check("B5 phán quyết hỏng", True, "đọc không được cũng không phải đạt",
          lambda: vp.write_text("{khong-phai-json", encoding="utf-8"))
    check("B0 ĐỐI CHỨNG: phán quyết tốt", False, "phải XANH",
...
def main() -> int:
    ...
    if "B" in args.lop:
        print("=== Lớp B: phán quyết và các lời từ chối của runner (không tốn model) ===")
        rows += layer_b()
    if "A" in args.lop:
