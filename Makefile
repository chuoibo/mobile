...
.DEFAULT_GOAL := help
.PHONY: help gate gate-merge test-db e2e up down clean db-reset logs ps migrate db-check seed demo demo-reset demo-check demo-data-check demo-persona-check demo-key-check demo-watch demo-watch-status demo-watch-install smoke
...
# Hai mục trên hỏi về DỮ LIỆU trên máy demo. Mục dưới hỏi về KHOÁ của nó, và đó
