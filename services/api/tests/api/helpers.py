...
def expense_payload(*, total=82000, description="Bữa tối"):
    return {
...
def propose_and_confirm(
    client, *, acknowledge=True, total=82000, description="Bữa tối"
):
    proposal = client.post(
        "/expenses", json=expense_payload(total=total, description=description)
    )
