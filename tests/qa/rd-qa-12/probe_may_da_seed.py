...
PY_BYTES = json.dumps({"display_name": GROUP_NAME}).encode("utf-8")           # seed / thanNhuSeed
JS_BYTES = json.dumps({"display_name": GROUP_NAME},
                      separators=(",", ":"), ensure_ascii=False).encode("utf-8")  # JSON.stringify

def post(label, raw):
    req = urllib.request.Request(
        f"{API}/contexts", data=raw, method="POST",
        headers={"Accept": "application/json", "Content-Type": "application/json",
                 "X-Actor-ID": str(MINH), "X-Actor-Roles": OWNER_ROLES,
                 "Idempotency-Key": KEY})
    try:
        with urllib.request.urlopen(req, timeout=30) as r:
            body = json.loads(r.read())
            print(f"{label:<34} -> {r.status}  id={body.get('id')}  replayed={r.headers.get('Idempotency-Replayed')}")
            return r.status
