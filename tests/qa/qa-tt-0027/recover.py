...
def labels(b):
    ps = b["places"]; return (sum(1 for p in ps if p["match"]["source"]=="ai"), len(ps))
def calls(): return get("/__qa_probe/count")["post"]
...
r1 = get("/places"); out["ngay_sau_storm"] = {"ai": labels(r1)[0], "tong": labels(r1)[1], "calls": calls()}
r2 = get("/places"); out["lan_ke_tiep"] = {"ai": labels(r2)[0], "calls": calls()}

print("... doi 65s de kiem cooldown 60s that su het han ...", flush=True)
time.sleep(65)
r3 = get("/places")
out["sau_65s_cooldown"] = {"ai": labels(r3)[0], "calls": calls(),
                           "ky_vong": "calls tang them 1 (hoi lai hang bi tu choi)"}
r4 = get("/places")
out["ngay_sau_do"] = {"ai": labels(r4)[0], "calls": calls(),
