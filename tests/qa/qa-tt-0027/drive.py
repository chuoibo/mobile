...
if ok:
    ...
    summary = {
        ...
        "refused_source": next(
            (p["match"]["source"] for p in places if p["id"] == cnt["refused_place"]),
            "KHONG-CO",
        ),
        "refused_score": next(
            (p["match"]["score"] for p in places if p["id"] == cnt["refused_place"]),
            None,
        ),
        "verdict_none_khi_source_none": all(
            p["match"]["verdict"] is None
            for p in places
            if p["match"]["source"] == "none"
        ),
    }
    ...
    json.dumps(
        {
            ...
            "http_ok": len(ok),
            "http_fail": len(bad),
            "MODEL_CALLS": cnt["post"],
            "wall_ms": wall,
            "loi": [b["err"] for b in bad][:3],
            "payload": summary,
        },
        ensure_ascii=False,
        indent=2,
