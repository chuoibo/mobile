...
export GEMINI_API_KEY=qa-probe-khong-phai-key-that QA_POST_DELAY=0.8
python3 -m uvicorn probe_app:app --port "$PORT" --log-level warning >$LOGDIR/rec-$PORT.log 2>&1 &
SRV=$!
for i in $(seq 1 60); do curl -sf "http://127.0.0.1:$PORT/healthz" >/dev/null 2>&1 && break; sleep 0.5; done
python3 "$HERE"/recover.py "http://127.0.0.1:$PORT"; RC=$?
